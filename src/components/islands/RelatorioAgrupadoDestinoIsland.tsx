...
async function fetchRelatorioDestinos(params: {
  ...
  const cacheKey = buildQueryLiteKey(["relatorioDestinos", qs.toString()]);
  const payload = await queryLite(
    cacheKey,
    async () => {
      const resp = await fetch(`/api/v1/relatorios/vendas-por-destino?${qs.toString()}`);
      if (!resp.ok) {
...
export default function RelatorioAgrupadoDestinoIsland() {
  ...
  async function carregarResumo(pageOverride?: number) {
    ...
    try {
      ...
      const rows = (await fetchRelatorioDestinos({
