...
const MOBILE_MONTH_WEEKDAYS = ["dom.", "seg.", "ter.", "qua.", "qui.", "sex.", "sáb."];
const MOBILE_WEEK_START_HOUR = 1;
const MOBILE_WEEK_END_HOUR = 23;
const MOBILE_WEEK_SLOT_HEIGHT = 52;
const MOBILE_WEEK_SLOT_GAP = 1;
...
export default function AgendaCalendar() {
  ...
  const dayMonthFormatter = React.useMemo(
    () => new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "2-digit", timeZone: "UTC" }),
    []
  );
  ...
  const dayNumberFormatter = React.useMemo(
    () => new Intl.DateTimeFormat("pt-BR", { day: "numeric", timeZone: "UTC" }),
    []
  );
  ...
  function formatWeekdayGoogle(date: Date) {
    return weekdayShortFormatter.format(date).trim().toLowerCase();
  }

  function formatWeekdayLong(date: Date) {
    return weekdayLongFormatter.format(date).trim().toLowerCase();
  }
  ...
  function syncMobileMonthState(date: Date) {
    const monthStart = new Date(date.getFullYear(), date.getMonth(), 1);
    const { gridStart, gridEnd } = getMonthGridRange(monthStart);
    setCurrentMonthTitle(formatMonthTitle(monthStart));
    setCurrentMonthKey(`${monthStart.getFullYear()}-${String(monthStart.getMonth() + 1).padStart(2, "0")}`);

    const inicio = toISODateLocal(gridStart);
    const fim = toISODateLocal(gridEnd);
    if (inicio && fim) {
    ...
    fetchHolidays(gridStart.getFullYear());
    if (gridEnd.getFullYear() !== gridStart.getFullYear()) {
      fetchHolidays(gridEnd.getFullYear());
    }
  }
  ...
  function buildMobileTimedBlocksForDay(dayDate: Date, sourceEvents: EventItem[]) {
    const dayStart = new Date(
      dayDate.getFullYear(),
      dayDate.getMonth(),
      dayDate.getDate(),
      MOBILE_WEEK_START_HOUR,
      0,
      0,
      0
    );
    ...
    return sourceEvents
      .filter((eventItem) => eventItem.display !== "background")
      .map((item) => {
        const rawStart = parseEventDate(item.start);
        const rawEnd = parseEventDate(item.end || item.start);
        const hasExplicitTime = item.start.includes("T") || item.end?.includes("T");
        const effectiveStart = hasExplicitTime ? rawStart : dayStart;
        const effectiveEnd = hasExplicitTime
          ? rawEnd > rawStart
            ? rawEnd
            : new Date(rawStart.getTime() + 30 * 60 * 1000)
          : new Date(dayStart.getTime() + 30 * 60 * 1000);

        if (effectiveEnd <= dayStart || effectiveStart >= dayEnd) return null;

        const clippedStart = effectiveStart < dayStart ? dayStart : effectiveStart;
        const clippedEnd = effectiveEnd > dayEnd ? dayEnd : effectiveEnd;
        const topMinutes = Math.max((clippedStart.getTime() - dayStart.getTime()) / 60000, 0);
        const durationMinutes = Math.max((clippedEnd.getTime() - clippedStart.getTime()) / 60000, 30);
        const slotUnit = MOBILE_WEEK_SLOT_HEIGHT + MOBILE_WEEK_SLOT_GAP;

        return {
          item,
          top: (topMinutes / 60) * slotUnit,
          height: Math.max((durationMinutes / 60) * slotUnit - MOBILE_WEEK_SLOT_GAP, 18),
        };
      })
      .filter(Boolean)
      .sort((a, b) => {
        if (!a || !b) return 0;
        return a.top - b.top;
      }) as Array<{ item: EventItem; top: number; height: number }>;
  }
  ...
  function renderEventContent(arg: any) {
    if (isMobile && arg?.view?.type === "dayGridMonth") {
      return <span className="agenda-month-event-chip-text">{arg.event.title || arg.timeText || ""}</span>;
    }
    return undefined;
  }
  ...
  function runAgendaCommand(action: "today" | "prev" | "next" | "dayGridMonth" | "timeGridWeek" | "timeGridDay" | "listWeek") {
    ...
    const isMobileMonthView = isMobile && currentViewType === "dayGridMonth";
    const monthReference = parseISODateLocal(`${currentMonthKey}-01`);

    if (isMobileMonthView && (action === "today" || action === "prev" || action === "next")) {
      const nextDate =
        action === "today"
          ? new Date()
          : new Date(
              monthReference.getFullYear(),
              monthReference.getMonth() + (action === "next" ? 1 : -1),
              1
            );
      syncMobileMonthState(nextDate);
      if (api) {
    ...
    if (!api) {
      if (isMobile && action === "dayGridMonth") {
        setCurrentViewType("dayGridMonth");
        syncMobileMonthState(new Date());
      }
      return;
    }

    if (isMobile && action === "dayGridMonth") {
      const nextDate = api.getDate?.() || monthReference || new Date();
      syncMobileMonthState(nextDate);
      api.gotoDate(nextDate);
      api.changeView("dayGridMonth");
    } else if (isMobileMonthView && (action === "timeGridWeek" || action === "timeGridDay" || action === "listWeek")) {
      api.gotoDate(monthReference);
  ...
  const mobileMonthGridDays = React.useMemo(() => {
    if (!viewportReady || !isMobile || currentViewType !== "dayGridMonth") return [];

    const monthStart = parseISODateLocal(`${currentMonthKey}-01`);
    ...
    const itemsByDay = new Map<string, EventItem[]>();
    ...
    sortedEvents.forEach((ev) => {
      ...
      if (endDay < gridStart || startDay > gridEnd) return;

      const cursor = new Date(Math.max(startDay.getTime(), gridStart.getTime()));
      const last = new Date(Math.min(endDay.getTime(), gridEnd.getTime()));

      while (cursor <= last) {
        const iso = toISODateLocal(cursor);
        if (iso) {
          const existing = itemsByDay.get(iso) || [];
          existing.push(ev);
          itemsByDay.set(iso, existing);
        }
    ...
    while (cursor <= gridEnd) {
      ...
      days.push({
        ...
        items: (itemsByDay.get(iso) || []).slice(0, 3),
      });
      cursor.setDate(cursor.getDate() + 1);
    }

    return days;
  }, [viewportReady, isMobile, currentViewType, currentMonthKey, events]);

  const mobileWeekDays = React.useMemo(() => {
    if (!viewportReady || !isMobile || currentViewType !== "timeGridWeek" || !visibleRange?.inicio || !visibleRange?.fim) {
      return [];
    }

    const start = parseISODateLocal(visibleRange.inicio);
    const end = parseISODateLocal(visibleRange.fim);
    ...
    const now = new Date();
    const timedEvents = events.filter((eventItem) => eventItem.display !== "background");

    const cursor = new Date(start);
    while (cursor <= end) {
      const dayEvents = buildMobileTimedBlocksForDay(cursor, timedEvents);

      days.push({
        iso: toISODateLocal(cursor),
        date: new Date(cursor),
        weekday: formatWeekdayGoogle(cursor),
        dayNumber: String(cursor.getDate()),
        isToday:
    ...
    return days;
  }, [viewportReady, isMobile, currentViewType, visibleRange?.inicio, visibleRange?.fim, events]);

  const mobileWeekHours = React.useMemo(
    () =>
      Array.from({ length: MOBILE_WEEK_END_HOUR - MOBILE_WEEK_START_HOUR + 1 }, (_, index) =>
        MOBILE_WEEK_START_HOUR + index
      ),
    []
  );

  const mobileDayData = React.useMemo(() => {
    if (!viewportReady || !isMobile || currentViewType !== "timeGridDay" || !visibleRange?.inicio) return null;

    const dayDate = parseISODateLocal(visibleRange.inicio);
    if (Number.isNaN(dayDate.getTime())) return null;

    const now = new Date();
    return {
      iso: toISODateLocal(dayDate),
      date: dayDate,
      weekday: formatWeekdayGoogle(dayDate),
      dayNumber: String(dayDate.getDate()),
      isToday:
        dayDate.getFullYear() === now.getFullYear() &&
        dayDate.getMonth() === now.getMonth() &&
        dayDate.getDate() === now.getDate(),
      events: buildMobileTimedBlocksForDay(dayDate, events),
    };
  }, [viewportReady, isMobile, currentViewType, visibleRange?.inicio, events]);
  ...
  return (
    <div className="agenda-page" style={{ display: "grid", gap: 16 }}>
      ...
      <div className="agenda-calendar-shell">
        ...
        {viewportReady && isMobile && currentViewType === "dayGridMonth" && (
          <div className="agenda-mobile-month" aria-label={`Calendário mensal de ${currentMonthTitle}`}>
            <div className="agenda-mobile-month-weekdays" aria-hidden="true">
              {MOBILE_MONTH_WEEKDAYS.map((label) => (
                <div key={label} className="agenda-mobile-month-weekday">
                  {label}
                </div>
              ))}
            </div>

            <div className="agenda-mobile-month-grid">
              {mobileMonthGridDays.map((day) => {
                const visibleItems = day.items.slice(0, 2);
                const remainingCount = Math.max(day.items.length - visibleItems.length, 0);

                return (
                  <div
                    key={day.iso}
                    className={`agenda-mobile-month-cell${day.isCurrentMonth ? "" : " is-outside"}${
                      day.isToday ? " is-today" : ""
                    }`}
                    role="button"
                    tabIndex={0}
                    onClick={() => openCreateModalAt(day.date, true)}
                    onKeyDown={(event) => {
                      if (event.key === "Enter" || event.key === " ") {
                        event.preventDefault();
                        openCreateModalAt(day.date, true);
                      }
                    }}
                  >
                    <div className="agenda-mobile-month-cell-number-row">
                      <span className="agenda-mobile-month-cell-number">{day.dayNumber}</span>
                    </div>

                    <div className="agenda-mobile-month-cell-events">
                      {visibleItems.map((item) => (
                        <button
                          key={`${day.iso}-${item.id}`}
                          type="button"
                          className="agenda-mobile-month-event"
                      ...
                      {remainingCount > 0 && (
                        <span className="agenda-mobile-month-more">+{remainingCount} mais</span>
                      )}
                    </div>
                  </div>
                );
        ...
        {viewportReady && isMobile && currentViewType === "timeGridWeek" && (
          <div className="agenda-mobile-week" aria-label={`Calendário semanal de ${currentMonthTitle}`}>
            <div className="agenda-mobile-week-header">
              <div className="agenda-mobile-week-axis-spacer" aria-hidden="true" />
              {mobileWeekDays.map((day) => (
                <div key={day.iso} className="agenda-mobile-week-day-header">
                  <span className="agenda-mobile-week-day-weekday">{day.weekday}</span>
                  <span className={`agenda-mobile-week-day-number${day.isToday ? " is-today" : ""}`}>
                    {day.dayNumber}
                  </span>
                </div>
              ))}
            ...
            <div className="agenda-mobile-week-body">
              <div className="agenda-mobile-week-axis">
                {mobileWeekHours.map((hour) => (
                  <div key={hour} className="agenda-mobile-week-axis-label">
                    {formatHourLabel(hour)}
                  </div>
                ))}
              </div>

              <div className="agenda-mobile-week-grid">
                {mobileWeekDays.map((day) => (
                  <div key={day.iso} className="agenda-mobile-week-column">
                    {mobileWeekHours.map((hour) => (
                      <button
                        key={`${day.iso}-${hour}`}
                        type="button"
                        className="agenda-mobile-week-slot"
                        onClick={() =>
                          openCreateModalAt(
                            new Date(day.date.getFullYear(), day.date.getMonth(), day.date.getDate(), hour, 0, 0, 0),
                            false
                          )
                        }
                        aria-label={`Adicionar evento em ${day.weekday} ${day.dayNumber} às ${formatHourLabel(hour)}`}
                      />
                    ))}

                    {day.events.map(({ item, top, height }) => (
                      <button
                        key={`${day.iso}-${item.id}-${top}`}
                        type="button"
                        className="agenda-mobile-week-event"
                        title={tooltipText(item)}
                        onClick={(event) => {
                          event.stopPropagation();
                          openDetailsModal(item);
                      ...
                      >
                        <span className="agenda-mobile-week-event-title">{item.title}</span>
                      </button>
                    ))}
                  </div>
                ))}
        ...
        {viewportReady && isMobile && currentViewType === "timeGridDay" && mobileDayData && (
          <div className="agenda-mobile-day" aria-label={`Calendário diário de ${currentMonthTitle}`}>
            <div className="agenda-mobile-day-header">
              <div className="agenda-mobile-day-header-line">
                <span className="agenda-mobile-day-weekday">{formatWeekdayLong(mobileDayData.date)}</span>
                <span className="agenda-mobile-day-date">{formatDateLong(mobileDayData.date)}</span>
              </div>
            </div>

            <div className="agenda-mobile-day-body">
              <div className="agenda-mobile-day-axis">
                {mobileWeekHours.map((hour) => (
                  <div key={hour} className="agenda-mobile-day-axis-label">
                    {formatHourLabel(hour)}
                  </div>
                ))}
              </div>

              <div className="agenda-mobile-day-grid">
                <div className="agenda-mobile-day-column">
                  {mobileWeekHours.map((hour) => (
                    <button
                      key={`${mobileDayData.iso}-${hour}`}
                      type="button"
                      className="agenda-mobile-day-slot"
                      onClick={() =>
                        openCreateModalAt(
                          new Date(
                            mobileDayData.date.getFullYear(),
                            mobileDayData.date.getMonth(),
                            mobileDayData.date.getDate(),
                            hour,
                            0,
                            0,
                            0
                      ...
                      aria-label={`Adicionar evento às ${formatHourLabel(hour)}`}
                    />
                  ))}

                  {mobileDayData.events.map(({ item, top, height }) => (
                    <button
                      key={`${mobileDayData.iso}-${item.id}-${top}`}
                      type="button"
                      className="agenda-mobile-day-event"
                      title={tooltipText(item)}
                      onClick={(event) => {
                        event.stopPropagation();
                        openDetailsModal(item);
                    ...
                    >
                      <span className="agenda-mobile-day-event-title">{item.title}</span>
                    </button>
                  ))}
                </div>
              </div>
        ...
        <div
          ...
          className={`${isMobile ? "agenda-calendar-mobile" : "agenda-calendar-desktop"}${
            viewportReady &&
            isMobile &&
            (currentViewType === "dayGridMonth" || currentViewType === "timeGridWeek" || currentViewType === "timeGridDay")
              ? " agenda-calendar-engine--hidden"
              : ""
          }`}
                  ...
                  ? {
                      timeGridWeek: {
                        dayHeaderContent: (args: any) => (
                          <>
                            <span className="agenda-week-header-weekday">{formatWeekdayGoogle(args.date)}</span>
                            <span className={`agenda-week-header-date${isTodayDate(args.date) ? " is-today" : ""}`}>
                              {formatDayNumber(args.date)}
                            </span>
                          </>
                        ),
                      },
                      ...
                      dayGridMonth: {
                        dayHeaderContent: (args: any) => (
                          <span className="agenda-month-header-weekday">{formatWeekdayGoogle(args.date)}</span>
                        ),
