...
export async function GET({ request }: { request: Request }) {
  try {
    ...
    let produtosQuery = client
      ...
      .order(all ? "nome" : "created_at", { ascending: all });
    ...
    const [
      ...
      client.from("paises").select("id, nome").order("nome"),
      client.from("subdivisoes").select("id, nome, pais_id").order("nome"),
      client
        .from("tipo_produtos")
        .select("id, nome, tipo")
        .eq("ativo", true)
    ...
    if (tipoResp.error) throw tipoResp.error;
    if (produtosResp.error) throw produtosResp.error;
    if (destinosProdutosErr) throw destinosProdutosErr;

    const produtoData = (produtosResp.data || []) as any[];

    const idsCidade = Array.from(new Set(produtoData.map((p) => p.cidade_id).filter(Boolean)));
    let cidadesData: any[] = [];
    if (idsCidade.length) {
      const { data: cidades, error: cidadesErr } = await client
        .from("cidades")
    ...
    if (companyId) {
      const { data: fornecedores, error: fornecedoresErr } = await client
        .from("fornecedores")
        .select("id, nome_completo, nome_fantasia")
        .eq("company_id", companyId)
        .order("nome_fantasia", { ascending: true });
      if (fornecedoresErr) throw fornecedoresErr;
      fornecedoresData = fornecedores || [];
    }

    const payload = {
      paises: paisData || [],
      subdivisoes: subdivisaoData || [],
      tipos: tipoResp.data || [],
      produtos: produtoData,
      total: all ? produtoData.length : produtosResp.count ?? produtoData.length,
      destinosProdutos: destinosProdutosData || [],
      cidades: cidadesData,
    ...
    console.error("Erro produtos/base", err);
    return new Response("Erro ao carregar base de produtos.", { status: 500 });
  }
}
