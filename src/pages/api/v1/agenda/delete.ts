  ...
  try {
    ...
    if (!isAdmin) {
      const denied = await requireModuloDelete(
        ...
        "Sem permissao para excluir agenda."
      );
    ...
    const rangeInicio = String(url.searchParams.get("range_inicio") || "").trim();
    const rangeFim = String(url.searchParams.get("range_fim") || "").trim();
    ...
    console.error("Erro agenda/delete", err);
    return new Response("Erro ao excluir evento.", { status: 500 });
  }
}
