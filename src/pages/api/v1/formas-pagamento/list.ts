  ...
  try {
    ...
    if (!isAdmin) {
      const denied = await requireModuloView(
        ...
        "Sem acesso a Formas de Pagamento."
      );
      if (denied) return denied;
    }

    const companyId = String((perfil as any)?.company_id || "").trim();
    const cacheKey = ["v1", "formasPagamento", "list", companyId || user.id].join("|");
    ...
    let query = client
      ...
      .select(
        "id, company_id, nome, descricao, paga_comissao, permite_desconto, desconto_padrao_pct, ativo, created_at"
      )
    ...
    console.error("Erro formas-pagamento/list", err);
    return new Response("Erro ao carregar formas de pagamento.", { status: 500 });
  }
}
