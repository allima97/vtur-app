...
export async function GET({ request }: { request: Request }) {
  try {
    ...
    const cacheKey = ["v1", "orcamentos", "tipos", user.id].join("|");
    ...
    await kvCache.set(cacheKey, data || [], 60);
    ...
    console.error("Erro orcamentos/tipos", err);
    return new Response("Erro ao carregar tipos.", { status: 500 });
  }
}
