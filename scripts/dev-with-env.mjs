#!/usr/bin/env node
/**
 * Dev script that ensures environment variables are available for `astro dev`.
...
function readIfExists(filePath, parser) {
  ...
  try {
    ...
    console.warn(`[dev-with-env] Falha ao ler ${path.basename(filePath)}: ${error.message}`);
    return {};
  }
}
...
function setupDevEnvironment() {
  ...
  if (missing.length > 0) {
    console.error(
      `[dev-with-env] Variáveis obrigatórias ausentes: ${missing.join(", ")}.`
    );
    console.error(
      "[dev-with-env] Configure em .env/.env.local ou no bloco [vars] do wrangler.toml."
    );
    process.exit(1);
  }
}
...
function runAstroDev() {
  ...
  const child = spawn("astro", ["dev", ...extraArgs], {
    cwd: projectRoot,
    env: process.env,
    stdio: "inherit",
  });
